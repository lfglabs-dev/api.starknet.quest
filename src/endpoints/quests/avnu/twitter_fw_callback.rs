...
pub async fn handler(
    ...
    let avnu_id = "1459844765822984193";
    ...
    let params = [
        ...
        (
            "redirect_uri",
            &format!(
                "{}/quests/avnu/twitter_fw_callback?addr={}",
    ...
    // Check if user is following AVNU
    ...
    loop {
        ...
        if next_token.is_none() || following_ids.contains(&avnu_id.to_string()) {
            break;
        }
    }

    if following_ids.contains(&avnu_id.to_string()) {
        ...
        get_error_redirect(
            error_redirect_uri,
            "You're not following AVNU's Twitter account".to_string(),
        )
    }
}
