...
pub async fn handler(
    ...
    let params = [
        ...
        (
            "redirect_uri",
            &format!(
                "{}/quests/twitter_tribe/twitter_fw_callback?addr={}",
    ...
    loop {
        ...
        if next_token.is_none()
            || following_ids.contains(&state.conf.quests.starknetid.account_id.to_string())
        {
            break;
        }
    }

    if following_ids.contains(&state.conf.quests.starknetid.account_id.to_string()) {
